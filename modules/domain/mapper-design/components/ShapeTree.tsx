...
const renderTitle = (item: Mapper, handleContextMenu: (evt: React.MouseEvent<HTMLDivElement>) => void) => {
  return (
    <Observer>
      {() => (
...
export const ShapeTree = observer(function ShapeTree(props: ShapeTreeProps) {
  ...
  const store = useLocalObservable(() => {
    const handleContextMenu = (item: Mapper) => (e: React.MouseEvent<HTMLDivElement>) => {
      e.stopPropagation();
      e.preventDefault();
